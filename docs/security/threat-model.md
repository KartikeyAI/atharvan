...
| Ambiguous price or provider substitution     | Commercial/provider       | Safe integer minor units, pricing-model invariants, exact Stripe Price binding, metadata and live-mode validation, environment isolation             | Invalid terms or drift are rejected before authority changes        |
| Duplicate charge or forged subscription      | Stripe billing            | Durable request before provider call, stable idempotency key, hosted Checkout, immutable bindings/revisions, scheduled exact-contract reconciliation | Duplicate sessions are prevented; mismatches remain visible drift   |
...
- Checkout cannot start for an inactive workspace, an unassigned plan, a free or
  contract plan, or a Price without the correct Stripe environment. Provider
  drift cannot rewrite the workspace plan or historical subscription evidence.
